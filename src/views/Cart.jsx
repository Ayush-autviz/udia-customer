...
import { StyleSheet, View, Text, SafeAreaView, ScrollView, TouchableOpacity, Image, TextInput, StatusBar, Alert } from 'react-native';
...
const Cart = (props) => {
  ...
  const check_payment_mode = () =>{
    console.log(props.payment_mode.slug,'slug');
    ...
    }else if(props.payment_mode.slug == 'stripe'){
      onCheckout()
    }
  }
  const createPaymentIntent = async () => {
    try {
      const response = await axios({
        method: 'post',
        url: api_url + 'customer/stripe_payment',
        data: { amount: props.total * 100, customer_id: global.id }
      });
  ...
  const onCheckout = async () => {
    ...
    try {
      ...
      console.log(data.client_secret, "payment intent");
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ padding:10 }}>
          ...
          <View style={{flexDirection:'row'}}>
            <View style={{ width:'10%',justifyContent:'center', alignItems:'flex-start' }}>
              <Icon type={Icons.Ionicons} name="timer-outline" color={colors.theme_fg} style={{ fontSize:20 }} />
            </View>  
        ...
        {order_type_index == 0 ?
          <View style={{ flexDirection:'row', paddingLeft:5 }}>
            <Text> Your Order Placing Time - {Moment().format('MMM DD, YYYY  hh:mm A')}</Text>
          </View>
          :
          <View style={{ flexDirection:'row', paddingLeft:5 }}>
            <Text> Your Order Placing Time - {Moment({current_date}).format('MMM DD, YYYY')} {time_format}</Text>
          </View>
        }
        <View style={{ margin:5 }} /> 
